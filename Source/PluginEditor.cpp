...
: AudioProcessorEditor (&p), audioProcessor (p), stepSequencer(8, audioProcessor.apvts, "step")
{
    setSize (800, 800);
    ...
    auto ratioRange = [](juce::Slider& s) { s.setRange(1.0, 16.0, 1.0); };
...
{
    // Get the current playback position and BPM from the playhead
    if (auto* playhead = audioProcessor.getPlayHead())
    {
        juce::AudioPlayHead::CurrentPositionInfo info;
        if (playhead->getCurrentPosition(info))
        {
            // Pass the ppqPosition and bpm to update the stepSequencer
            stepSequencer.updateFromHostPosition(info.ppqPosition, info.bpm);
        }
    }
}
