...
{
    sr = sampleRate;
    delayLineReal.prepare({ sampleRate, (juce::uint32)samplesPerBlock, 2 });
    delayLineReal.setMaximumDelayInSamples(100);
    delayLineImag.prepare({ sampleRate, (juce::uint32)samplesPerBlock, 2 });
...
void WahProcessor::updateParameters()
{
    ...
    float freeFreq   = pfloat("wahFreq");
    float feedback   = pfloat("wahFeedback");
    float delay      = pfloat("wahDelay");
    float offset = *apvts.getRawParameterValue("Offset");
    // Modulation switches
    ...
    const bool offsetModOn = *apvts.getRawParameterValue("Offset5modulate"); // new modulation switch


    // Step sequencer value
    int currentStep = static_cast<int>(pfloat(PID::seqCurrentStep[4]));
    float stepValue = pfloat(PID::seqStep[4][juce::jlimit(0, 7, currentStep)]);

    // --- Update internal member variables directly ---
    lfoFrequency[0] = freqModOn ? juce::jlimit(1.0f, 10.0f, freeFreq + stepValue * 50.0f) : freeFreq;

    lfoFrequency[1] = offsetModOn ? lfoFrequency[0]*offset * stepValue : lfoFrequency[0]*offset;
    
    lfoStartPhase = 0.0f; // default, can modify if needed
    feedbackAmount = feedbackModOn ? juce::jlimit(0.0f, 1.0f, feedback + stepValue) : feedback;
    delayLength = juce::jlimit(5, 100, static_cast<int>(delay));
    if (delayModOn)
        delayLength = juce::jlimit(5, 100, static_cast<int>(delay + stepValue * 100.0f));
    
//    channelLfoOffset[0] = 0.0f; // left channel, no offset
//    channelLfoOffset[1] = offsetModOn ? stepValue * offset : offset;
}

float WahProcessor::processSample(float in, int channel)
{
    const float twoPi = juce::MathConstants<float>::twoPi;

    // --- per-channel LFO value ---
    float lfoVal = 1.0f + std::cos(lfoPhase[channel] + lfoStartPhase);
    std::complex<float> cLocal(
        std::cos(lfoVal) * feedbackAmount,
        std::sin(lfoVal) * feedbackAmount
    );

    // --- read delayed samples for this channel ---
    float delayedReal = delayLineReal.popSample(channel);
    float delayedImag = delayLineImag.popSample(channel);
    std::complex<float> delayed(delayedReal, delayedImag);
    std::complex<float> input(in, 0.0f);

    // --- core wah processing ---
    auto outComplex = cLocal * delayed + (1.0f - feedbackAmount) * input;

    // --- push new delayed samples ---
    delayLineReal.setDelay(delayLength);
    delayLineImag.setDelay(delayLength);
    delayLineReal.pushSample(channel, outComplex.real());
    delayLineImag.pushSample(channel, outComplex.imag());

    // --- clamp output ---
    float outSample = outComplex.real() * 3.0f;
    outSample = juce::jlimit(-32768.0f, 32767.0f, outSample);

    // --- advance LFO phase per channel ---
    lfoPhase[channel] += lfoFrequency[channel] * twoPi / static_cast<float>(sr);
    if (lfoPhase[channel] >= twoPi)
        lfoPhase[channel] -= twoPi;
